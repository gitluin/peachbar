...
To start, create `~/.sbar/`. Then, I symlink the files from this repository to my custom `/ibin` directory (which is added to `$PATH`) with the prefix `sbar_` added to all but `startsara.sh` and `saratagline.sh`. Then I add `/ibin/start_sbar.sh &` to my `~/.xinitrc` **before** the `exec sara`/`startsara.sh` line. Make sure you change the directory names, etc. to match your system.
